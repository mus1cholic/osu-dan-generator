...
class OsuFileGenerator:
    ...
    def add_timing_points(self, timing_points_arr: list[str], start_time: int, end_time: int, fade_in_start_time: int, offset: int):
        ...
        uninherited_timings_range_start_index = bisect.bisect_left(uninherited_timings, start_time)
        uninherited_timings_range_end_index = bisect.bisect_left(uninherited_timings, end_time)

        if len(uninherited_timings) == 1:
            uninherited_timings_range_start_index = 0
            uninherited_timings_range_end_index = 0
        else:
            if uninherited_timings_range_start_index == len(uninherited_timings):
                uninherited_timings_range_start_index -= 1
            if uninherited_timings_range_end_index == len(uninherited_timings):
                uninherited_timings_range_end_index -= 1

        test1 = uninherited_timings[uninherited_timings_range_start_index]

        inherited_timings = [int(time.split(",")[0]) for time in inherited_timings_points_arr]

        inherited_timings_range_start_index = bisect.bisect_left(inherited_timings, start_time)
        inherited_timings_range_end_index = bisect.bisect_left(inherited_timings, end_time)

        if len(inherited_timings) == 1:
            inherited_timings_range_start_index = 0
            inherited_timings_range_end_index = 0
        else:
            if inherited_timings_range_start_index == len(inherited_timings):
                inherited_timings_range_start_index -= 1
            if inherited_timings_range_end_index == len(inherited_timings):
                inherited_timings_range_end_index -= 1

        test2 = inherited_timings[inherited_timings_range_start_index]

        timings = []
        for i in range(uninherited_timings_range_start_index, uninherited_timings_range_end_index + 1):
            timings.append(uninherited_timings_points_arr[i])
        for i in range(inherited_timings_range_start_index, inherited_timings_range_end_index + 1):
            timings.append(inherited_timings_points_arr[i])

        def custom_cmp(timing1, timing2):
            # red lines before green lines
        ...
        for timing in timings:
            ...
            cur_timing_point_split[0] = str(int(cur_timing_point_split[0]) + offset - (test1 if int(cur_timing_point_split[6]) == 1 else test2) + fade_in_start_time)
    ...
    def add_hit_objects(self, hit_objects_arr: list[str], start_time: int, end_time: int, fade_in_start_time: int, offset: int):
        ...
        test = hitobjects[hitobjects_range_start_index]
